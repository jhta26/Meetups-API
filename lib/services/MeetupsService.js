class MeetupsService {
  ...
  async findAllInvolvedMeetups(userId) {
    try {
      const allMeetups = await this._meetupsRepository.findAllInvolvedMeetups(
        userId
      );
      return allMeetups;
    } catch (error) {
  ...
  async postByUserId(attributes, authenticatedUserId) {
    ...
    console.log('MEETUPSSERVICE', attributes);
    try {
      const newmeetups = await this._meetupsRepository.create({ attributes });
      console.log('MEETUPSSERVICE2', newmeetups);
      return newmeetups;
    } catch (error) {
      throw error;
    }
  ...
  async update(id, attributes, authenticatedUserId) {
    ...
    console.log('MEETUPSSERVICE',id,attributesToUse,authenticatedUserId)
